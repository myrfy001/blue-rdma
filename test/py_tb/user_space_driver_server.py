...
class UserspaceDriverServer:
    def __init__(self, listen_addr, nic_a, listen_port_a: int,
        ...
        if isinstance(nic_a, mock_host.MockNicAndHost):
            self.driver_listen_port_a = listen_port_a
            self.simulator_listen_port_a = listen_port_a + 1
        if isinstance(nic_b, mock_host.MockNicAndHost):
    ...
    def run(self):
        ...
        if isinstance(self.mock_nic_a, mock_host.MockNicAndHost):
        ...
        if isinstance(self.mock_nic_b, mock_host.MockNicAndHost):
    ...
    def _run(self, listen_addr, listen_port,
             mock_nic: mock_host.MockNicAndHost):
...
if __name__ == "__main__":
    ...
    mock_nic_b = mock_host.MockNicAndHost(
        host_mem,
        host=listen_addr,
        port=listen_simulator_port_b,
        rx_packet_wait_time=0.001)
